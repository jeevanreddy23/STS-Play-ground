...
test("contains the complete STS pile inspection workflow", async () => {
  const [page, css, migration, schema] = await Promise.all([
  ...
  assert.match(page, /square handle to resize/);
  assert.match(page, /onPointerMove/);
  assert.match(page, /marker-resize-handle/);
  ...
  assert.match(schema, /markerSizePx/);
